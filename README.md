...
```
2. Откройте `data/settings.json` и заполните свои ключи. Прописывать порт или путь к базе данных **необязательно** — Docker сам переопределит их через переменные окружения.
